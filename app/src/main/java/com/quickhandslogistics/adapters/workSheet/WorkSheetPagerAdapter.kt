...
class WorkSheetPagerAdapter(
    ...
    private var onGoingWorkItemsCount =
        if (!allWorkItemLists?.first.isNullOrEmpty()) allWorkItemLists?.first?.size else 0
    private var cancelledWorkItemsCount =
        if (!allWorkItemLists?.second.isNullOrEmpty()) allWorkItemLists?.second?.size else 0
    private var completedWorkItemsCount =
        if (!allWorkItemLists?.third.isNullOrEmpty()) allWorkItemLists?.third?.size else 0
    private var unfinishedWorkItemsCount =
        if (!allWorkItemLists?.third.isNullOrEmpty()) allWorkItemLists?.fourth?.size else 0
    private var notOpenWorkItemsCount =
        if (!allWorkItemLists?.third.isNullOrEmpty()) allWorkItemLists?.fifth?.size else 0

    private var ongoingFragment =
        WorkSheetItemFragment.newInstance(resources.getString(tabTitles[0]), allWorkItemLists)
    private var completedFragment =
        WorkSheetItemFragment.newInstance(resources.getString(tabTitles[1]), allWorkItemLists)
    private var cancelledFragment = WorkSheetItemFragment.newInstance(
        resources.getString(tabTitles[2]),
        allWorkItemLists,
        containerGroupNote
    )
    private var unfinishedFragment = WorkSheetItemFragment.newInstance(
        resources.getString(tabTitles[3]),
        allWorkItemLists,
        unfinishedNotes
    )
    private var notOpenFragment = WorkSheetItemFragment.newInstance(
        resources.getString(tabTitles[4]),
        allWorkItemLists,
        notOpenNotes
    )
    ...
    override fun getPageTitle(position: Int): CharSequence? {
        val count =
            if (position == 0) onGoingWorkItemsCount else if (position == 1) completedWorkItemsCount else if (position == 2) cancelledWorkItemsCount else if (position == 3) unfinishedWorkItemsCount else notOpenWorkItemsCount
